...
### PR #22 deployment review — 2026-09-13

The site now waits for the explicit site-migrate service. Its two-migration baseline handles an initialized database with missing tables, skips already present original tables, and fails startup on a migration error. It uses the same DATABASE_URL as the site and does not apply Analytics DB migrations. Future site migrations must be added explicitly to this baseline runner.

The analytics-retention service runs the existing 90-day cleanup after analytics becomes healthy and every 86400 seconds thereafter. It has only the analytics storage network and Analytics DB URL; cleanup failure exits for the container restart policy. This change schedules cleanup; it does not implement backup scheduling.

Synthetic PostgreSQL acceptance passed: existing database upgrade, repeat migration preserving data, migration failure, database separation, 91-day deletion/89-day preservation, and the declared Compose loop's daily delay/failure behavior. The schedule test uses deterministic command doubles rather than a two-day wait. Reproduce with node scripts/verify-analytics-upgrade.mjs <isolated-docker-config-directory> after analytics:build. Twelve browser lifecycle tests, changed-file ESLint, TypeScript and analytics build passed. The BFCache review assumption was not reproduced: identity.stop clears the session, so restore reinitializes and opens a page. The lifecycle test additionally confirms distinct page IDs, clicks after restoration and subsequent navigation. No production migration or rollout was performed.
