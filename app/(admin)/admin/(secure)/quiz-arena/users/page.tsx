...
import { fetchUsers, type UserListSortBy } from "@/lib/quiz-arena-api";
...
export default function UsersPage() {
  ...
  const { data, isLoading, isFetching, error, dataUpdatedAt } = useQuery<UsersData>({
  ...
  return (
    <main className="min-w-0 space-y-6 py-2 fade-in">
      {selectedUser !== null ? <UserDetail key={selectedUser} userId={selectedUser} onClose={() => setSelectedUser(null)} /> : null}
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <div className="w-[17rem] min-w-[17rem]">
                          <p className="font-semibold text-[#1f4257]"><button className="underline" onClick={() => setSelectedUser(item.id)}>{formatDisplayName(item)}</button></p>
                          <p className="mt-1 text-xs text-ember/60">
