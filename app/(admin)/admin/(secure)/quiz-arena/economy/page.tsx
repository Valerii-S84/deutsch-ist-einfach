...
import { fetchEconomyCohorts, fetchEconomyPurchases, fetchEconomySubscriptions } from "@/lib/quiz-arena-api";
...
function readPercent(row: CohortRow, weekOffset: number): number | null {
...
export default function EconomyPage() {
  ...
  const cohortAverages = useMemo(() => {
    if (!cohortRows.length) {
      return { w0: null, w4: null, w8: null };
    }
    const average = (weekOffset: number): number | null => {
      const values = cohortRows.map((row) => readPercent(row, weekOffset));
  ...
  if (purchasesQuery.error || subscriptionsQuery.error || cohortsQuery.error || !purchases || !subscriptionsQuery.data || !cohortsQuery.data) return <QueryState error={purchasesQuery.error || subscriptionsQuery.error || cohortsQuery.error} loading={isLoading} />;

  return (
    <main className="space-y-6 py-2 fade-in">
      <DataFreshness receivedAt={Math.min(purchasesQuery.dataUpdatedAt, subscriptionsQuery.dataUpdatedAt, cohortsQuery.dataUpdatedAt)} />
        ...
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <article className="surface rounded-2xl p-4">
              ...
              <p className="mt-2 text-3xl">{totals.purchasesTotal > 0 ? formatCurrency(totals.avgPurchaseEur) : "Nicht definiert (keine Käufe)"}</p>
            </article>
            <article className="surface rounded-2xl p-4">
              <p className="text-xs uppercase tracking-wide text-ember/65">Premium-Anteil</p>
              <p className="mt-2 text-3xl">
                {totals.revenueStars > 0 ? `${totals.premiumSharePercent.toLocaleString("de-DE", { maximumFractionDigits: 1 })}%` : "Nicht definiert (kein Umsatz)"}
              </p>
            </article>
            <article className="surface rounded-2xl p-4">
              <p className="text-xs uppercase tracking-wide text-ember/65">Premium mit Status ACTIVE (max. 500)</p>
          ...
          <section className="surface rounded-2xl p-4">
            ...
            <p className="mt-1 text-sm text-ember/70">
              Ungewichteter Mittelwert der letzten 8 angezeigten Kohorten. Fehlende Wochen sind keine Nullwerte. Käufe je Woche seit Registrierung; noch nicht erreichte Wochen sind im Backend ebenfalls 0.
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <article className="rounded-xl border border-ember/15 bg-white/70 p-3">
                ...
                <p className="mt-2 text-2xl">
                  {cohortAverages.w0 === null ? "Keine Daten" : `${cohortAverages.w0.toLocaleString("de-DE", { maximumFractionDigits: 1 })}%`}
                </p>
              </article>
              <article className="rounded-xl border border-ember/15 bg-white/70 p-3">
                <p className="text-xs uppercase tracking-wide text-ember/60">Woche 4</p>
                <p className="mt-2 text-2xl">
                  {cohortAverages.w4 === null ? "Keine Daten" : `${cohortAverages.w4.toLocaleString("de-DE", { maximumFractionDigits: 1 })}%`}
                </p>
              </article>
              <article className="rounded-xl border border-ember/15 bg-white/70 p-3">
                <p className="text-xs uppercase tracking-wide text-ember/60">Woche 8</p>
                <p className="mt-2 text-2xl">
                  {cohortAverages.w8 === null ? "Keine Daten" : `${cohortAverages.w8.toLocaleString("de-DE", { maximumFractionDigits: 1 })}%`}
                </p>
              </article>
            </div>
            {cohortRows.length > 0 ? (
                          ...
                          return (
                            <td key={`${row.cohort_week}-${offset}`} className="py-2">
                              <p className="text-xs text-ember/80">
                                {value === null ? "Keine Daten" : `${value.toLocaleString("de-DE", { maximumFractionDigits: 1 })}%`}
                              </p>
          ...
          <section className="surface overflow-x-auto rounded-2xl p-4">
            <h2 className="text-xl">Premium mit Status ACTIVE (max. 500)</h2>
