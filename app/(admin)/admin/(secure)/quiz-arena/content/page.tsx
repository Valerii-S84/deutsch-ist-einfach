...
import { fetchContentHealth } from "@/lib/quiz-arena-api";
...
export default function ContentPage() {
  const { data, isLoading, error, dataUpdatedAt } = useQuery<ContentHealthData>({
    queryKey: ["content"],
              ...
              {sortedLevelStats.map((item) => (
                <article key={item.level} className="rounded-xl border border-ember/15 bg-white/70 p-3">
                  ...
                  <p className="mt-2 text-xs text-ember/70">
                    Versuche je Frage (%): {item.coverage_percent.toLocaleString("de-DE")}%
                  </p>
