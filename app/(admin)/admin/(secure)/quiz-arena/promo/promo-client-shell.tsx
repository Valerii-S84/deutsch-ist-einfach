...
export function PromoClientShell() {
  ...
  const required = [promoState.promoQuery, promoState.productsQuery, promoState.sessionQuery, ...(promoState.detailId !== null ? [promoState.detailQuery, promoState.statsQuery, promoState.auditQuery] : [])];
  const failed = required.find((query) => query.error);
  if (failed || required.some((query) => !query.data)) return <QueryState error={failed?.error} loading={required.some((query) => query.isLoading)} />;

  return (
    <main className="space-y-6 py-2 fade-in">
      <DataFreshness receivedAt={Math.min(...required.map((query) => query.dataUpdatedAt))} />
