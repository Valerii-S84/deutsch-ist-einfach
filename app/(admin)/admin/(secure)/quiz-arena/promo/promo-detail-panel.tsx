...
type PromoDetailPanelProps = {
  ...
  onRevealCode: (promoId: number | string) => void;
  onCopyRevealedCode: (value: string) => void;
  onEdit: (item: PromoItem) => void;
  onToggle: (promoId: number | string) => void;
  onRevoke: (promoId: number | string) => void;
};
          ...
          {detailTab === "params" ? (
            <div className="grid gap-4 text-sm sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-2">{detail?.max_total_uses ?? "Keine Daten"}</p>
              </div>
              <div>
            ...
            <div className="space-y-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <article className="rounded-2xl border border-emerald-200/70 bg-emerald-50 p-4">
                  ...
                  <p className="mt-2 text-3xl">{stats?.used_total ?? "Keine Daten"}</p>
                </article>
                <article className="rounded-2xl border border-amber-200/70 bg-amber-50 p-4">
                  <p className="text-xs uppercase tracking-[0.2em] text-amber-700">
                    Aktive Reservierungen
                  </p>
                  <p className="mt-2 text-3xl">{stats?.reserved_active ?? "Keine Daten"}</p>
                </article>
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                {PROMO_TOTAL_STATUSES.map((status) => (
                  <article key={status} className="rounded-2xl border border-ember/10 bg-white p-4">
                    ...
                    <p className="mt-2 text-2xl">{stats?.status_totals?.[status] ?? "Keine Daten"}</p>
                  </article>
                ))}
              </div>
