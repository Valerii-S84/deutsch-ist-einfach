...
export function usePromoClientState() {
  ...
  const createMutation = useMutation({
    ...
    onError: () =>
      setFlash({ kind: "error", text: "Speichern nicht bestätigt. Vor Wiederholung Liste und Audit prüfen." }),
  });
  ...
  const bulkMutation = useMutation({
    ...
    onError: () =>
      setFlash({ kind: "error", text: "Generierung nicht bestätigt. Vor Wiederholung Liste und Audit prüfen." }),
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: () =>
      setFlash({ kind: "error", text: "Änderung nicht bestätigt. Vor Wiederholung Details und Audit prüfen." }),
  });

  const toggleMutation = useMutation({
    mutationFn: async (promoId: number | string) => {
    ...
    onError: () =>
      setFlash({ kind: "error", text: "Statusänderung nicht bestätigt. Vor Wiederholung Details und Audit prüfen." }),
  });

  const revokeMutation = useMutation({
    mutationFn: async ({ promoId, reason }: { promoId: number | string; reason: string | null }) => {
    ...
    onError: () =>
      setFlash({ kind: "error", text: "Widerruf nicht bestätigt. Vor Wiederholung Reservierungen und Audit prüfen." }),
  });
  ...
  const revealMutation = useMutation({
    ...
    onSuccess: async (data) => {
      if (data.id === detailId) setRevealedCode(data.raw_code);
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ["promo-detail", detailId] }),
        queryClient.invalidateQueries({ queryKey: ["promo-audit", detailId] }),
      ]);
  ...
  async function handleRevoke(promoId: number | string) {
  ...
  function openDetailWithParams(promoId: number | string) {
    setDetailId(promoId);
    setDetailTab("params");
    setRevealedCode(null);
  }

  function openDetailWithStats(promoId: number | string) {
    setDetailId(promoId);
    setDetailTab("stats");
    setRevealedCode(null);
  }
