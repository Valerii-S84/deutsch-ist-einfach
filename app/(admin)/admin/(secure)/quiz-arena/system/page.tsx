...
import { fetchSystemHealth } from "@/lib/quiz-arena-api";
...
export default function SystemPage() {
  ...
  const workerCount = data?.services.celery?.workers?.length ?? "Keine Daten";
  const webhookUpdates = data?.services.bot_webhook?.processed_updates_15m ?? "Keine Daten";
  const queueAvailable = data?.services.redis?.ok === true;
  const servicesOkCount = serviceEntries.length - downServices.length;

  if (error || !data) return <QueryState error={error} loading={isLoading} />;

  return (
    <main className="space-y-6 py-2 fade-in">
      <DataFreshness receivedAt={dataUpdatedAt} />
      <header className="surface rounded-2xl p-5">
        <h1 className="text-3xl">Systemstatus</h1>
        <p className="mt-2 text-sm text-ember/70">
          Backend-Status: Keine Webhook-Updates in 15 Minuten beweisen keinen Ausfall. Queue celery ist eine Warteschlangenlänge, kein Fehlerzähler.
        </p>
      </header>
        ...
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article className="surface rounded-2xl p-4">
              ...
              <p className={`mt-2 text-2xl ${downServices.length === 0 ? "text-emerald-700" : "text-red-700"}`}>
                {serviceEntries.length === 0 ? "Keine Daten" : downServices.length === 0 ? "Gemeldete Dienste stabil" : "Achtung"}
              </p>
            ...
            <article className="surface rounded-2xl p-4">
              ...
              <p className="mt-2 text-2xl">{queueAvailable ? data.queue_stats.pending.toLocaleString("de-DE") : "Keine Daten"} wartend</p>
              <p className="mt-1 text-xs text-ember/70">
                {queueAvailable ? data.queue_stats.failed.toLocaleString("de-DE") : "Keine Daten"} in Queue celery
              </p>
            </article>
            <article className="surface rounded-2xl p-4">
                ...
                <article key={key} className="rounded-xl border border-ember/15 bg-white/70 p-3 text-sm">
                  ...
                  {key === "celery" ? (
                    <p className="mt-2 text-xs text-ember/75">
                      Worker: {value.workers?.length.toLocaleString("de-DE") ?? "Keine Daten"}
                    </p>
                  ) : null}
                  {key === "bot_webhook" ? (
                    <p className="mt-2 text-xs text-ember/75">
                      Updates (15m): {value.processed_updates_15m?.toLocaleString("de-DE") ?? "Keine Daten"}
                    </p>
                  ) : null}
                  {!value.ok ? (
          ...
          <section className="grid gap-4 lg:grid-cols-2">
            <article className="surface rounded-2xl p-4">
              ...
              <p className="mt-2 text-sm">Wartend: {queueAvailable ? data.queue_stats.pending.toLocaleString("de-DE") : "Keine Daten"}</p>
              <p className="text-sm">Queue celery: {queueAvailable ? data.queue_stats.failed.toLocaleString("de-DE") : "Keine Daten"}</p>
              {data.queue_stats.failed > 0 ? (
                <p className="mt-3 text-sm text-red-700">
                  Fehlgeschlagene Jobs vorhanden. Worker-Log und Retry-Flow pruefen.
                </p>
              ) : (
                <p className="mt-3 text-sm text-emerald-700">Keine in Queue celeryen Jobs im Moment.</p>
              )}
