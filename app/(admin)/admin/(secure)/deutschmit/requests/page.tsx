...
export default function ContactRequestsPage() {
  ...
  return <main className="surface space-y-4 rounded-2xl p-5">
    <DeutschmitNavigation active="requests" />
    <h2 className="text-3xl">Контактні заявки deutschmit.de</h2>
    <p>Керування заявками збережене окремо від аналітики. Тексти форм не надсилаються в аналітичні події.</p>
    <select aria-label="Джерело заявок" disabled={mutation.isPending} value={source} onChange={event => { setSource(event.target.value); setPage(1); setNotice(""); }}><option value="site">Власна БД сайту</option><option value="legacy">Архів сайту в Quiz Arena backend</option></select>
