...
function request(token?: string, query = "") { return new NextRequest(`https://site.example.test/api/admin/analytics/deutschmit/sessions/${TEST_ID}${query}`, { headers: token ? { Cookie: `${SITE_ADMIN_SESSION_COOKIE}=${token}` } : {} }); }
