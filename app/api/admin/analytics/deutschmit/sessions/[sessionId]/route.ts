...
export async function GET(request: NextRequest, { params }: { params: Promise<{ sessionId: string }> }) {
  ...
  if (!id.success || request.nextUrl.search) {
    return Response.json({ error: "invalid_session_request" }, { status: 400, headers });
  }
  try {
    return Response.json(await readAnalyticsSession(id.data), { headers });
  } catch (error) {
